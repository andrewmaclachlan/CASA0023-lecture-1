...
Electromagnetic radiation(EMR) (e.g. from the sun) isn't automatically reflected. Experiences a number of changes prior to hitting the sensor
These include:
...
However, this is dependent on the incident (incoming) radiation so it must be calibrated with a pure white reference panel...
...
* Identified how constrations (envrionmental or sensor) can dictate the choice of data we use. 
